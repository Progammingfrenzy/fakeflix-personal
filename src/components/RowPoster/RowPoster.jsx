...
import { addRecentlyWatched } from "../../redux/continueWatching/continueWatching.actions";
...
const RowPoster = (result) => {
  ...
  const saveToContinueWatching = () => {
    const watchedItem = {
      id: item.id,
      media_type: media_type || (title ? "movie" : "tv"),
      title: fallbackTitle,
      poster_path,
      backdrop_path,
      timestamp: Date.now(),
    ...
    console.log("Dispatching watched item:", watchedItem);
    dispatch(addRecentlyWatched(watchedItem));
  };
  ...
  return (
    ...
    >
      {isLarge ? (
        poster_path ? <img src={`${BASE_IMG_URL}/${poster_path}`} alt={fallbackTitle} /> : ""
      ) : backdrop_path ? (
